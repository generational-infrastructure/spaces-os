...
in
{
  ...
  config = {
    ...
    # Per-permission Wayland gating. Patches niri so restricted protocol
    # globals (layer-shell, data-control, screencopy, virtual-keyboard, …)
    # are gated per security-context app-id against the map at
    # /etc/spaces/wayland-permissions.txt (generated by the apps module).
    # Behaviour-preserving when the map is empty: restricted clients are
    # denied every wayland.* global, exactly as upstream's single binary
    # `client_is_unrestricted` filter.
    #
    # Set via the package (not a nixpkgs overlay): `runNixOSTest` pins
    # `nixpkgs.pkgs` per node, which makes `nixpkgs.overlays` unsettable
    # there. mkDefault lets test-support swap in a niri that *also* carries
    # the software-rendering patch (see modules/nixos/test-support).
