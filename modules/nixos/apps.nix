...
let
  ...
  # Wayland-permissions map consumed by the patched niri
  # (`patches/niri-per-permission-gating.patch`, applied via
  # `modules/nixos/niri.nix`). Lines are `<appId> <permission>` pairs,
  # one per `wayland.*` permission
  ...
  waylandPermissionsFile = pkgs.writeText "spaces-wayland-permissions.txt" ''
    ...
    # patched niri (see patches/niri-per-permission-gating.patch,
    # applied via programs.niri.package in modules/nixos/niri.nix) reads this at
    # startup to gate restricted Wayland-protocol globals per app-id.
