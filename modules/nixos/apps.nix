...
let
  ...
  # Wayland-permissions map consumed by patched niri (when
  # `patches/niri-per-permission-gating.patch` is wired into
  # `programs.niri.package`). Lines are `<appId> <permission>` pairs,
  ...
  waylandPermissionsFile = pkgs.writeText "spaces-wayland-permissions.txt" ''
    ...
    # globals per app-id. Until that patch is wired into
    # programs.niri.package the file is informational only.
