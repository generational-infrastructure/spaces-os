# Hermes Desktop (Electron) app build + wrapper.
#
# VENDORED from hermes-agent nix/desktop.nix @ v2026.7.20 with ONE change:
# the electron node-headers FOD hash below matches OUR nixpkgs electron
# (upstream pins a hash for THEIR nixpkgs' electron — with
# hermes-agent.inputs.nixpkgs.follows, upstream's own desktop package is
# unbuildable whenever the electron versions drift). cli.nix builds this
# file via callPackage with upstream's passthru internals
# (hermesNpmLib/hermesAgent); everything else is byte-identical — diff
# against upstream when bumping the input, and update the hash via
#   nix hash file --sri --type sha256 <headers.tar.gz>
#
