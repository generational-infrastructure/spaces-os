...
#
# Double-function: `inputs` = the publisher's flake inputs (see vms.nix).
{ inputs }:
{
  config,
  lib,
...
let
  ...
  # Electron desktop app: OUR nixpkgs electron + upstream's npm-built
  # renderer, assembled by the vendored ./desktop.nix (upstream's own
  # `desktop` package pins a node-headers hash for THEIR nixpkgs'
  # electron and breaks under nixpkgs follows). hermesNpmLib/the agent
  # package come from upstream's default-package passthru.
  desktopPackage =
    let
