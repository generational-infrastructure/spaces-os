...
}:
{
  ...
  microvm = {
    ...
    qemu.extraArgs = lib.optionals cfg.gpu.enable [
      ...
      "-device"
      "virtio-gpu-gl-pci,hostmem=${cfg.gpu.hostmem},blob=true,venus=true"
    ];
  ...
  boot.blacklistedKernelModules = lib.mkIf cfg.gpu.enable (
    lib.mkForce [
      "rfkill"
      "intel_pstate"
    ]
  ...
  services.hermes-agent = {
    ...
    settings = lib.recursiveUpdate llamaSwapSettings cfg.settings;
