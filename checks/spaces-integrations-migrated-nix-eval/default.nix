...
let
  ...
  integrations = system.config.services.spaces-integrations.integrations;
  defOf =
    name:
...
assert caldav.autoRun == [ "list" "get" "etag" ];
...
assert contacts.autoRun == [ "discover" "search" "get" ];
...
assert mail.autoRun == [ "envelope_list" "message_read" ];
