...
# checks/pi-session-ws (headless, no VM). x86_64-linux only (runNixOSTest needs
# kvm + nixos-test); other systems get a trivial stub.
{ pkgs, inputs, ... }:

if pkgs.stdenv.hostPlatform.system != "x86_64-linux" then
  pkgs.runCommand "pi-chat-remote-x86_64-only" { } "mkdir -p $out"
else
