...
#
# x86_64-linux only: the `test-machine` host is x86-pinned and
# pkgs.testers.runNixOSTest requires a builder with `kvm + nixos-test`
# features, which the aarch64 CI worker does not currently advertise.
# Other systems get a trivial stub so `nix flake check` stays green.
{ pkgs, inputs, ... }:

if pkgs.stdenv.hostPlatform.system != "x86_64-linux" then
  pkgs.runCommand "test-machine-x86_64-only" { } "mkdir -p $out"
else
