#!/usr/bin/env python3
"""Mock pi-sessiond for the restart-preserves-model contract test.
...
so the driver can assert the restart sequence on the wire:

  hello           -> welcome {caps:{executor: <id>}}
  list_sessions   -> sessions [ current list ]
  create_session  -> attached {sessionId}   (fresh id; the create ack)
  ...
  command         -> recorded; set_memory is tolerated (PiSession sends it
                     on every spawn, buffered so it lands first after
                     attach); set_model is acked with an event-envelope
                     {type:"response", command:"set_model", success:true,
                      data:{provider, id}, id:<echo>} so _request-based
                     setModel paths (setModelAndWait) resolve.
...
async def handler(ws, *_):
    ...
    async for raw in ws:
        ...
        if kind == "hello":
            ...
            # set_memory arrives on every spawn (buffered, first after
            # attach) — tolerate it: recorded above, no reply needed.
            if ptype == "set_model":
                resp = {
                    "type": "response",
                    "command": "set_model",
