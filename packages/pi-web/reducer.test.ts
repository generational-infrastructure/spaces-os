...
function delta(d: string) {
  return { type: "message_update", assistantMessageEvent: { type: "text_delta", delta: d } };
}

test("a user prompt is appended verbatim", () => {
  const s = withUserPrompt(emptyState(), "hello");
  expect(s.messages).toEqual([{ role: "user", text: "hello", streaming: false }]);
});
...
test("a streamed reply accumulates deltas into one assistant bubble", () => {
  ...
  s = withPiEvent(s, { type: "message_update", assistantMessageEvent: { type: "text_start" } });
  for (const d of ["Hello", ", ", "world", "!"]) s = withPiEvent(s, delta(d));
  s = withPiEvent(s, { type: "agent_end" });

  expect(s.typing).toBe(false);
  expect(s.messages).toHaveLength(1);
  expect(s.messages[0]).toEqual({ role: "assistant", text: "Hello, world!", streaming: false });
});
...
test("text_delta without a text_start still opens one streaming bubble", () => {
  ...
  expect(s.messages).toEqual([{ role: "assistant", text: "ab", streaming: true }]);
});
...
test("an extension_ui_request opens a pending confirm; a confirm method only", () => {
  ...
  expect(s.confirms).toEqual([{ id: "sc-1", title: "Run it?", state: "pending" }]);

  // Non-confirm methods don't add a confirm bubble.
  s = withPiEvent(s, { type: "extension_ui_request", id: "sc-2", method: "input" });
  expect(s.confirms).toHaveLength(1);

  // Duplicate id is ignored (replays).
  s = withPiEvent(s, { type: "extension_ui_request", id: "sc-1", method: "confirm" });
  expect(s.confirms).toHaveLength(1);
});

test("answering a confirm sets allowed/denied", () => {
  let s = withPiEvent(emptyState(), { type: "extension_ui_request", id: "x", method: "confirm" });
...
test("sidechannel_resolved collapses only a still-pending confirm", () => {
  ...
  let answered = withConfirmAnswer(
    withPiEvent(emptyState(), { type: "extension_ui_request", id: "y", method: "confirm" }),
    "y",
    true,
  );
