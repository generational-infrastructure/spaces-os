...
def test_malformed_line_then_connection_survives(client):
    ...
    resp = client.rpc("tools/list", req_id=3)
    assert resp["id"] == 3
...
def test_clone_to_workspace_extracts_tree(client, env):
    ...
    assert gets
    assert gets[0][2].get("Authorization") == f"Bearer {TOKEN}"
...
def test_open_pull_request_pushes_workspace_edits(client, env):
    ...
    dest = Path(env["share"]) / "hello"
    (dest / "AGENT.md").write_text("agent was here\n")
    resp = call_tool(
        client,
        "open_pull_request",
...
def test_schema_json_matches_advertised_tools_and_needs():
    ...
    assert schema["tools"] == [t["name"] for t in integration_github.TOOLS]
    fields = {**schema["config"], **schema["secrets"]}
    required = {k for k, v in fields.items() if v["required"]}
    assert required == set(integration_github._NEEDS)
