...
# -*- coding: utf-8 -*-
#
# Distro fork of calamares-nixos-extensions/modules/nixos/main.py.
#
# Differences vs upstream:
#   - emits `/mnt/etc/nixos/flake.nix` + `flake.lock` referencing the
#     distro flake. flake.nix points `inputs.distro.url` at upstream
#     `github:generational-infrastructure/distro`; flake.lock is
#     generated at install time via `nix flake lock --override-input
#     distro path:<staged> --override-input distro/<name> path:<staged>
...
#     cfggnome/plasma6/...) — distro module owns the desktop;
#   - drops upstream-opinion blocks (cfgfirefox, cfgmisc, cfgpkgs,
#     cfgtail, cfgunfree, cfglatestkernel) so the distro module is the
...
#     Calamares; everything else from `distro.nixosModules.distro`;
#   - autologin is wired through `services.greetd` (what distro uses)
#     rather than `services.displayManager.autoLogin`.
#
# Boot/LUKS/btrfs-subvol logic preserved verbatim from upstream — those
# are correctness-critical and not distro-specific.
...
# modules in the generated flake.nix, alongside `distro.nixosModules.distro`.

cfghead = """# Host-specific overlay generated by the distro graphical installer.
# Distro defaults come from `distro.nixosModules.distro` (wired in via
...
# Distro auto-logs into niri via greetd. Point greetd at the user
# Calamares created (overrides distro's `lib.mkDefault "alice"`).
...
# upstream `github:generational-infrastructure/distro`. The matching
...
flake_template = """{
  description = "Auto-generated by the distro graphical installer.";

  # Upstream source of truth. Run `nix flake update distro` to refresh.
  inputs.distro.url = "github:generational-infrastructure/distro";

  outputs = inputs: {
    nixosConfigurations.@@hostname@@ = inputs.distro.lib.mkSystem {
      system = "@@system@@";
      hostName = "@@hostname@@";
      modules = [
        ./configuration.nix
...
def render_flake_nix(gs):
    ...
    # If /etc/distro-test-support exists, include the test-support module
    ...
    if os.path.exists("/etc/distro-test-support"):
        extra = "        inputs.distro.nixosModules.test-support\n"
    else:
        extra = ""
...
def setup_luks_keyfile(gs, root_mount_point):
    ...
    Mirrors upstream verbatim — correctness-critical, not distro-specific.
    """
...
def run():
    ...
    # unfree-extraModulePackages stripping is dropped — distro
    ...
    # Generate flake.lock by overriding every input distro declares
    ...
    # post-install `nix flake update distro` rewrites the lock back to
    # canonical github.
    ...
    nixFlakeLockCmd.extend(
        [
            ...
            "--override-input",
            "distro",
            "path:" + DISTRO_FLAKE_PATH,
        ]
    )
    for name, path in INPUT_OVERRIDES.items():
        nixFlakeLockCmd.extend(["--override-input", "distro/" + name, "path:" + path])
    try:
